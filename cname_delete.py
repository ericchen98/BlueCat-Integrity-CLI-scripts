...
'''
cname_delete -c config1 -v view1 -z test.corp -r www -i host.test.corp

    -c config name
    -v view name
    -z zone name
    -r record_name(CName record name)
    -i linkedRecordName(the host record)
...
# main function
# ---------------------------------------------------------------------
def cname_delete(*args):    

    # no used (been corrected with work with cli.py)
    # convert args to string (for logging)
    # passed_args = ''
    # for tmp in args:
    #    passed_args = passed_args + ' '+ tmp 
    #
    # passed_args = " [Calling arguments]: cname_delete " 
    ...
    # because cname don't allow duplicate name (that link to different host record), it is 
    # ok to delete cname without checking linkedRecordName

    linkedRecordName = rdata

    #if (linkedRecordName == None) or (linkedRecordName[0:1] == "-"):
                    ...
                    if len(result_list) == 0: # no entity found
                        ...
                        for ent in result_list:
                            ...
                            if debug_api:
                                ...
                                # 'properties': 'absoluteName=*.google.com.block.rpz.easycard.com.tw|
                                #                linkedRecordName=to.rpz.easycard.com.tw|'
                                # }
