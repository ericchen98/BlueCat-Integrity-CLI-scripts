...
# v5.4-20220705 allow add record with dot (properties = "parentZoneName=%s" % (zone) )
...
'''
srv-add -c config1 -v view1 -z test.corp -r srv1 -i host1.test.corp 
srv-add -c config1 -v view1 -z test.corp -r srv1 -i host1.test.corp -o 10 -p 5060 -w 100
srv-add -c config1 -v view1 -z test.corp -i host1.test.corp -o 10 -p 5060 -w 100 -k 
    ...
    note
     -i "linkedRecordName" need to be an existing "host record" or an existing "external record" in BAM
'''
import getopt
import sys
import json
...
def srv_add(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "hdc:v:z:r:i:m:u:o:p:w:k",["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "hdc:v:z:r:i:m:u:o:p:w:k",["debug=","version"])
        ...
        if configuration:
            ...
            if view:
                ...
                if zone:
                    ...
                    # ------------------------------------------------ 
                    # just add it. If record exists, api will return "Duplicate of another item"
                    # ------------------------------------------------ 
                    # for adding naptr
                    # to enable add record name with dot (properties needs to have parentZoneName)
                    properties = "parentZoneName=%s" % (zone)
                    
                    ttl = "-1"
                    try:
                        rtn = bam.addSRVRecord( viewid, add_absolutename, priority, port, weight, \
                            linkedRecordName, ttl, properties)
                        logMsg =  "Success: Added%srecord %s" % ( sameAsZone_Message,
                                    absolutename)
                        print_log.go(log_info, logMsg)                    
                    except ValueError as e:
                        logMsg = "Failure: %s Failed to add%srecord %s" % ( str(e), sameAsZone_Message, 
                                        absolutename)
                        print_log.go(log_error, logMsg)
                        BAM.bam_logout(bam)
                        return(1)
