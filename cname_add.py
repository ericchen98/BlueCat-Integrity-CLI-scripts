...
# BAM ver: 26.1
#
# v1.2-20260521 remove debug print
# v1.1-20230828 fixed log does not show in cli.py issue 
# v1.0-20210921 (use BAM.py v20210918-1735)
...
def cname_add(*args):
    ...
    passed_args = " Input: cname_add " + passed_args
    '''
    ...
    help_txt='''
cname_add -c config1 -v view1 -z test.corp -r www -i host.test.corp
    ...
    # add a cname record and link it to a host record
    # note: a cname record cannot be "same as zone" (a zone apex always
    # carries SOA/NS records, and CNAME can't coexist with anything else
    # at the same name), so there is no -k option here.
    '''
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:z:r:i:t:h", ["debug=","version"])
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:z:r:i:t:h", ["debug=","version"])
            ...
            if view:
                ...
                if zone:
                    ...
                    try:
                        ...
                        ent_id = bam.addAliasRecord( viewid, add_absolutename, linkedRecordName,
                                                        ttl ,properties)

                        logMsg = "Success: added%srecord %s linkedRecordName:%s" % (sameAsZone_Message,
                                absolutename, linkedRecordName)
