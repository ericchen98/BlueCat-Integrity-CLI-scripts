...
'''
host-query -c config1 -v view1 -z test.corp -r host1  (query host record)
host-query -c config1 -v view1 -z test.corp -k        (query "same as zome host" record)
    -c config_name
    -v view_name
    -z zone_name
    -r record_name
...
def host_query(*args):    
    ...
    # -------------------------------------------------------------------------------
    # first setp: define log_prog_type_index : to be called in BAM.print_log()
    # -------------------------------------------------------------------------------
    prog_type_view = 0
    prog_type_add = 1
    prog_type_delete = 2
    ...
    # will cause sameAsZone=None (because "-K" becomes -r's parameter. (rrName=="-k")
        ...
        if configuration:
            ...
            if view:
                ...
                if zone:
                    ...
                    result_list = bam.getEntitiesByName(zoneid, rrName, "HostRecord", 0, max_obj_count)
                    ...
                    if len(result_list) == 0: # no entity found
                        ...
                        for ent in result_list:
                            ...
                            if sameAsZone:
                                printRecordPrompt = "Found same as zone record: "
                            else:
                                printRecordPrompt = "Found record: "
                                
                            logMsg = "Success: found%srecord %s IP %s" % (sameAsZone_Message, 
                                                properties['absoluteName'] , properties['addresses'])
                            print_log.go(log_info, logMsg)
                            
                        if RecordPrintCount == 0:
                            logMsg = "Failure: No%srecord found: %s" % (sameAsZone_Message, absolutename)
                            print_log.go(log_error, logMsg)
                            BAM.bam_logout(bam)
                            return(1)
