...
def srv_query(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:z:r:i:o:p:w:kh", ["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:z:r:i:o:p:w:kh", ["debug=","version"])
        ...
        if configuration:
            ...
            if view:
                ...
                if zone:
                    ...
                    # hostid = bam.addHostRecord(viewid, absolutename, rdata, -1, properties)
                    ...
                    # when use getEntitiesByName(), if no record found, will return "[]" (len(rtn_json)) is zeron
                    # print(result_list)
                    # output: []
                    ...
                    if len(result_list) == 0: # no entity found
                        ...
                        # logMsg2 is to build final message in log
                        ...
                        for ent in result_list:
                            ...
                            print ( "Record: " + properties['absoluteName'] + ","  \
                                    + " type: " + "SRV" + "," \
                                    + " linked-to: " + properties['linkedRecordName'] + "," \
                            ...
                            if RecordPrintCount == 1:
                                # don't print first "|"
                                logMsg = " i:%s p:%s o:%s w:%s " % (properties['linkedRecordName'], \
                        ...
                        # print to console                    
                        print( "Success: Found%srecord %s " % (sameAsZone_Message, absolutename ) )
                        
                        # print to to log only ( append all rdata together )
