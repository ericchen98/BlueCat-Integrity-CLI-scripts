...
#
#
# v1.1 20260924 - forked from cli.py: added tab-completion for command names
#                 in the interactive shell (readline on Linux/Mac, pyreadline3
#                 fallback on Windows if installed).
#                 need module:
...
# for command history (up/down arrow) and tab-completion of command names.
# Python's stdlib "readline" module only exists on Linux/Mac (it wraps the
# GNU Readline/libedit C library, which isn't part of a standard Windows
# Python install) - on Windows, "pyreadline3" (a separate pip package,
# `pip install pyreadline3`) reimplements the same readline API on top of
# the Windows console APIs, so the exact same set_completer()/
# parse_and_bind() calls below work unchanged on either platform once it's
# installed. If neither is available, the shell still works - it just
# falls back to no history/no tab-completion, same as before.
...
# ---------------------------------------------------------------------
# tab-completion: every command word cli() recognizes below (the actual
# commands, e.g. "a-add", plus the topic words that print a help_* block,
# e.g. "a", "host"), so pressing Tab after a partial word like "host-a"
# completes it the same way it would in a real shell.
# ---------------------------------------------------------------------
ALL_COMMANDS = sorted(set([
    # meta / shell commands
    "help", "h", "?", "dir", "version", "exit", "bamconfig-show",
    # help topics (typing just the topic word prints its help_* block)
    "config", "view", "zone", "cname", "ns", "srv", "naptr", "generic",
    "a", "host", "dns", "matchclient", "server", "deploy", "mac", "rpz",
    # user / access
    "access-rights", "password", "password-encrypt",
    # config/view/zone
    "config-add", "config-query",
    "view-add", "view-query", "view-update",
    "zone-add", "zone-query", "zone-query-all", "zone-update", "zone-rr-query",
    "matchclient-add", "matchclient-delete", "matchclient-query", "matchclient-update",
    "dnsrole-add", "dnsrole-delete",
    # DNS records
    "a-add", "a-update", "a-query", "a-delete",
    ...
    "rpz-add", "rpz-delete", "rpz-upload", "rpz-update",
    # server / deploy
    "server-query", "deploy-server", "deploy-status",
    # MAC address / pool
    "macpool-add", "macpool-delete", "macpool-query",
    "macpoolitem-add", "macpoolitem-delete", "mac-query",
]))


def _make_completer(options):
    """Return a readline-compatible completer: on each Tab press, readline
    calls completer(text, state) with state=0,1,2,... until it gets None
    back, to walk through every option whose prefix matches what's typed
    so far (case-insensitively, since cli()'s own commands are matched
    case-insensitively too)."""
    def completer(text, state):
        text_lower = text.lower()
        matches = [o for o in options if o.startswith(text_lower)]
        try:
...
def _setup_tab_completion():
    """Wire up Tab-completion for the interactive shell prompt, if a
    readline-compatible module is available (see the import block near the
    top of this file). Safe to call even if it isn't - callers should only
    call this when _readline_available is True."""
    readline.set_completer(_make_completer(ALL_COMMANDS))
    # "tab: complete" is the standard GNU Readline binding for hooking Tab
    # to the completer above - pyreadline3 supports the same call on
    # Windows.
    readline.parse_and_bind("tab: complete")
    # treat "-" as a normal word character (so completing "host-a" doesn't
    # stop at the hyphen and only try to complete the "a" part).
...
def cli(*args):
    ...
    # enable Tab-completion of command names, only in the interactive
    # shell prompt (not when a single command was passed on the command
    # line) and only if a readline-compatible module actually imported.
    if shell_mode and _readline_available:
