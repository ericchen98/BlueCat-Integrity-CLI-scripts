...
#
'''
macPool-add -c config1 -p macPool1 [-i]

    -c config_name
    -p macPool
    -i enable instant Deployment for changes to this MAC Pool 
...
def macPool_add(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:p:ih", ["debug=","version"])           
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:p:ih", ["debug=","version"])
    ...
    try:
        if configuration:
            ...
            if macPool:
                ...
                if macPoolid != 0:
                    logMsg = "Failure: MacPool %s exists already" % (macPool)
                    print_log.go(log_error, logMsg)
                    BAM.bam_logout(bam)
                    return(1)
                else:
                    if insDeploy:
                        data={"id":0,"name":macPool,"type":"MACPool", 'properties': 'instantDeploy=true|'}
                    else:
                        data={"id":0,"name":macPool,"type":"MACPool"}
                    rtn=bam.addEntity(confid,data)
                    
                    if rtn !=0:
                        logMsg =  "Success: MacPool %s added. ObjId= %s " % (macPool, rtn)
                        print_log.go(log_info, logMsg)
                    else:
                        logMsg =  "Failure: MacPool %s add failure" % (macPool)
                        print_log.go(log_info, logMsg)
