...
#
# program name: dhcp4range_query.py
# by: Eric Chen (hchen@bluecatnetworks.com)
# python ver: python 3
# BAM ver: 26.1
#
# v1.0-20250912 created 
#


'''
dhcp4range_query -c config1 -i 10.1.1.0/24 -s 10.1.1.11 -e 10.1.1.20
...
def dhcp4range_query(*args): 
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:i:s:e:",["ni=","debug=","dry", "version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:i:s:e:",["ni=","debug=","dry", "version"])
            ...
            if confid == 0:
               ...
               # ent found, but need to tell whether it is in same config!!!
                ...
                if len(result_list) == 0: # no entity found
                    ...
                    if debug_api:
                        print ('---------------- debug: IP4Network Entity read ----------')
                        print (ent_list)
                        print ('Found IP4Network obj count: %s' %  len(ent_list) )
                    ...
                    # {'id': 100925, 'name': None, 'type': 'IP4Network', 
                    #    'properties': 'CIDR=10.1.1.0/24|allowDuplicateHost=disable|pingBeforeAssign=disable|
                    # inheritAllowDuplicateHost=true|inheritPingBeforeAssign=true
                    # |inheritDNSRestrictions=true|inheritDefaultDomains=true
                    # |inheritDefaultView=true|gateway=10.1.1.1|dynamicUpdate=false|'}
                    
                    ip4Network_id = ent['id'] 
    
                    try:
                        type = "DHCP4Range"
                        ent_dhcp4range = bam.getEntityByRange(ip4Network_id, start_ip, end_ip, type)
                        
                        #logMsg = "Success: added dhcp v4 Range. Network: %s start ip:%s End ip: %s" % (
                        #        ipNetwork,start_ip, end_ip)
                        
                        # ----------------------------------------------------------------------
                        # Returned DHCP Range Object (No "DHCP alert settings"?)
                        # ----------------------------------------------------------------------
                        # {'id': 100964, 'name': 'test dhcp range', 'type': 'DHCP4Range', 
                        #   'properties': 'start=10.2.2.30|end=10.2.2.40|
                        #                  SplitStaticAddresses=false|
                        #                  excludeDHCPRange=10.2.2.33-10.2.2.35,10.2.2.37-10.2.2.38,|'}
                        #

                        if debug_api:
                            print ('---------------- debug: IP4Range Entity read ----------')
                            print (ent_dhcp4range)

                        if ent_dhcp4range['id'] == 0:
                            logMsg = "Failure: dhcp range: %s-%s (ip4network:%s) not found" % ( 
                                    ip4Network_id, start_ip, end_ip)
                            print_log.go(log_error, logMsg)
                            ...
                            prop = bam.splitProp(ent_dhcp4range)
                            
                            id_to_print = str(ent_dhcp4range['id'])
                            name = ent_dhcp4range['name']
                            start = prop['start']
                            end = prop['end']
                            SplitStaticAddresses = prop['SplitStaticAddresses']
                            excludeDHCPRange = prop['excludeDHCPRange']
                            '''        
                            logMsg =  ( "Success: Found dhcp v4 Range."  \
                                    + "id:" + id_to_print + ","  \
                                    + "name:" + name + ","  \
                                    + "start:" + start + ","  \
                                    + "end:" + end + "," \
                                    ...
                                    + "Parent Network:"+ ipNetwork )
                            '''        
                            
                            #logMsg = "Success: Found dhcp v4 Range. id:%s, name:%s, start:%s, end:%s,"  + \
                            #         "SplitStaticAddresses:%s, excludeDHCPRange:(%s), Parent Network:%s" % ( \
                            #    id_to_print, name, start, end, SplitStaticAddresses, excludeDHCPRange, ipNetwork)
                            
                            logMsg1 = "Success: Found dhcp v4 Range. id:%s, name:%s, start:%s, end:%s,"  + \
                                     "SplitStaticAddresses:, excludeDHCPRange:(%s), Parent Network:%s" 
                                     
                            logMsg = logMsg1 % ( id_to_print, name, start, end,  excludeDHCPRange, ipNetwork)
