...
#
# program name: view-update.py
# by: Eric Chen (hchen@bluecatnetworks.com)
# python ver: python 3
# BAM ver: 26.1
#
# v1.2-20230828 fixed log does not show in cli.py issue 
# v1.1-20221226 change log dir to "./"
# v1.0-20220424 new create
#

'''
view-update -c config1 -v view1 --nv=viewNew1
...
def view_update(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:",["nv=","debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:",["nv=","debug=","version"])
    ...
    if (not new_view) or (new_view[0:1] == "-"):  
        logMsg = "Failure: --nv new_view_name option is required"
        print_log.go(log_error, logMsg)
        return(1)
        
    if (view == new_view):  
        logMsg = "Failure: -v and --nv cannot be same"
        print_log.go(log_error, logMsg)
        return(1)
        ...
        if configuration:
            ...
            if view:
                ...
                if viewid == 0:
                    ...
                    view_ent['name'] = new_view
                    
                    # update ent
                    bam.update (view_ent)
            
                    logMsg =  "Success: Update old view name: %s to new view name: %s" % ( view, new_view)
