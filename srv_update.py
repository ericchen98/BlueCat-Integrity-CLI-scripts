...
'''
srv-update -c config1 -v view1 -z test1.corp -r srv1 -i host1.test.corp -o 10 -p 5060 -w 100 --ni=host2.test.corp --no=20 --np=2020 --nw=200
    -c config name
    -v view name
    -z zone name
    -r record name
    -i linkedRecordName  --ni=newlinkedRecordName
    -o priority          --no=newPriority
    -p port              --np=newPort
    -w weight            --nw=newWeight
...
def srv_update(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "hdc:v:z:r:i:m:u:o:p:w:k",["ni=","no=","np=","nw=","debug=","version"])
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "hdc:v:z:r:i:m:u:o:p:w:k",["ni=","no=","np=","nw=","debug=","version"])
    ...
    for o,v in opts:
        ...
        elif o == "-i": linkedRecordName = v
        elif o == "-o": priority = v
        elif o == "-p": port = v
        elif o == "-w": weight = v
        elif o == "--ni": newlinkedRecordName = v  
        elif o == "--no": newpriority  = v  
        elif o == "--np": newport = v  
        elif o == "--nw": newweight = v  
    ...
    # Parameter checking --- for SRV record only ----------------
    if (not linkedRecordName) or (linkedRecordName[0:1] == "-"):
        logMsg = "Failure: -i linkedRecordName option is required"
        print_log.go(log_error, logMsg)
        return(1)

    # Parameter checking --- for SRV record only ----------------
    if (not newlinkedRecordName) or (newlinkedRecordName[0:1] == "-"):
        logMsg = "Failure: --ni newlinkedRecordName option is required"
        print_log.go(log_error, logMsg)
        return(1)
    ...
    # note issue: found if user enter -r but don't give record name, e.g.
    #    -c config1 -v view1 -z test.corp  -i 10.10.104 -r -k
            ...
            if view:
                ...
                if zone:
                    ...
                    if convertRR:
                        ...
                        if not (rrName == None):
                            rrName = rrName.lower()
                        if not (linkedRecordName == None):
                            linkedRecordName = linkedRecordName.lower()
                        if not (newlinkedRecordName == None):
                            newlinkedRecordName = newlinkedRecordName.lower()
                    ...
                    try:
                        result_list = bam.getEntitiesByName(zoneid, rrName, "SRVRecord", 0, max_obj_count)
                    ...
                    if len(result_list) == 0: # no entity found
                        ...
                        for ent in result_list:                 # ent is also json too
                            ...
                            if debug or debug_api:
                                ...
                                #print ('properties=')
                                #print (properties)
                                print ('--------------------------------------absoluteName(exist vs input)')
                                print (properties['absoluteName'])
                                print (absolutename)
                                print ('--------------------------------------linkedRecordName(exist vs input)')
                                print (properties['linkedRecordName'])
                                print (linkedRecordName)
                                print ('--------------------------------------priority(exist vs input)')
                                print (properties['priority'])
                                print (priority)
                                print ('--------------------------------------port(exist vs input)')
                                print (properties['port'])
                                print (port)
                                print ('--------------------------------------weight(exist vs input)')
                                print (properties['weight'])
                                print (weight)

                            if properties['linkedRecordName'] == linkedRecordName and \
                                properties['priority'] == priority and \
                                properties['port'] == port and \
                                properties['weight'] == weight :
                            
                                print ( "(*) Record: " + properties['absoluteName'] + ","  \
                                                + " link to record: " + properties['linkedRecordName'] + "," \
                                ...
                                # update prop by new parameters
                                
                                properties['linkedRecordName'] = newlinkedRecordName 
                                properties['port'] = newport 
                                ...
                                try:
                                    ...
                                    record_changed = False
                                    error_Logged = True
                                    # cannot run return(1) here as we want to pring the rest of records
                        ...
                        if record_changed:
                            print ()
                            logMsg =  "Success: update%srecord %s " % ( sameAsZone_Message, 
                                                                    absolutename)
