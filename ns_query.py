...
# v5.4-20221209
#       1) modified: logPath = "./"
#       2) add flag convertRR = False to convert RR Name, rdata and  to lower cases.
...
#
'''
ns-query -c config1 -v view1 -z test.corp -r pc1 
ns-query -c config1 -v view1 -z test.corp -k
            ...
            if view:
                ...
                if zone:
                    ...
                    if len(result_list) == 0: # no entity found
                        ...
                        # log message to be appended all found IP
                        ...
                        for ent in result_list:
                            ...
                            # print (properties)
                            #     output : {'type': 'NS', 'absoluteName': 'a1.test.corp', 'rdata': '10.0.0.1'}
                            # if "absoluteName" = zone name (e.g. "test.corp" ), this is "same as zone" record
                            # {'id': 171898, 'name': '', 'type': 'GenericRecord', 'properties': 
                            #    'absoluteName=test.corp|type=NS|rdata=9.9.9.9|'}
                            ...
                            if properties['type']=="NS":
                                ...
                                print ("Info Success: Found%srecord: %s " % 
                                        (sameAsZone_Message, properties['rdata']) )
                          
                                # build log (adding results to one line)
                                if RecordPrintCount == 1:
                                    logMsg = "i:%s" % ( properties['rdata'] )
                                else:
                                    logMsg = "|i:%s" % ( properties['rdata'] )

                            #end if properties['type']=="NS":
                        ...
                        if RecordPrintCount != 0:
                            ...
                            #logMsg= "Success: Found%srecord %s " % (sameAsZone_Message, absolutename )
                            #print_log.go(log_error, logMsg)
                        else:
                        ...
                        # 2021-07-09 21:37:10,eric,vDNS,,cli,View,"[INFO]: Success: Found record a1.test.corp 
                        # i:1.1.1.10|i:1.1.1.11|i:1.1.1.5|i:1.1.1.4|i:1.1.1.3|i:1.1.1.2 | 
                        # i:1.1.1.1 "," Input: a-query-t7 -c config1 -v view1 -z test.corp -r a1"
