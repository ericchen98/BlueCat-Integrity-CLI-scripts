...
#
'''
macPool-query -c config1 -p macPool1
...
def macPool_query(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:p:h", ["debug=","version"])           
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:p:h", ["debug=","version"])
    ...
    try:
        if configuration:
            ...
            if macPool:
                ...
                if macPoolid == 0:
                    logMsg = "Failure: Failed to find view: %s" % (macPool)
                    print_log.go(log_error, logMsg)
                    BAM.bam_logout(bam)
                    return(1)
                else:
                    logMsg = f"Success: found MAC Pool: {macPool} objId:{macPoolid}"
