...
'''
mac-query -c config1 -m 112233445566
...
def mac_query(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:m:h", ["debug=","version"])           
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:m:h", ["debug=","version"])
    ...
    try:
        if configuration:
            ...
            if mac:
                ...
                macid = mac_ent['id']
                if macid == 0:
                    logMsg = "Failure: Failed to find Mac Address: %s" % (mac)
                    print_log.go(log_error, logMsg)
                    BAM.bam_logout(bam)
                    return(1)
                else:
                    logMsg = f"Success: found Mac Address:{mac} ObjId:{macid}"
