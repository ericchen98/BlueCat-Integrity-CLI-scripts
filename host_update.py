...
# v5.4-20220402 rename host-update-all.py to host-update.py to meet backward compatibility 
...
#
# debug switch (debut function is testing only)
#   -d         # print additonal user info
#   --debug=1  # print script debug print
#   --debug=2  # print bam.py debug info e.g. API http request

'''
host-update -c config1 -v view1 -z test.corp -r host1 -i 10.10.10.3
host-update -c config1 -v view1 -z test.corp -r host1 -i 10.10.10.3,10.10.10.4 -p yes
host-update -c config1 -v view1 -z test.corp -i 10.10.10.3 -k 
    ...
    -r record_name
    -i rdata
    -p [yes|no]     ('yes':add ptr. 'No':remove ptr. if -p is not set, will keep original setting)
    -k (same as zome record. When -k is used, -r is ignored)

    Note: update a host record host1 with 2 IP addresses 10.10.10.3 and 10.10.10.4 
           use "," to seperate IP addresses, NO space between IP address and ","
'''
import getopt
import sys
import json
...
def host_update(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:z:r:i:kp:h", ["debug=","version"])
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:z:r:i:kp:h", ["debug=","version"])
        ...
        if configuration:
            ...
            if view:
                ...
                if zone:
                    ...
                    #if len(ent) == 0:  # no existing record found. Do nothing, exist
                    # when not found, ent is: (is not empty)
                    # {'id': 0, 'name': None, 'type': None, 'properties': None}
                    ...
                    if ent['id'] == 0:  # no existing record found. Do nothing, exist
                        ...
                        # Found host record, update IP
                        
                        properties = bam.splitProp(ent)
                        
                        properties['addresses'] = rdata
                        ...
                        try:
                            bam.update (ent)
                            logMsg =  "Success: Update%srecord %s IP %s PTR=%s" % ( 
                                        sameAsZone_Message, absolutename, rdata, ptr_info)
                            print_log.go(log_info, logMsg)
                            record_changed = True
                        except ValueError as e:
                            #raise ValueError, "Failed to update record: %s" % absolutename
                            logMsg = "Failure: %s Failed to update%srecord: %s" % ( str(e), sameAsZone_Message, absolutename)
                            print_log.go(log_error, logMsg)
                            BAM.bam_logout(bam)
                            return(1)
                    #end if ent['id'] == 0:
