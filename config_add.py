...
# 
# program name: config-add.py (changed from add-confzone.py)
...
def config_add(*args):   
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:h", ["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:h", ["debug=","version"])
    ...
    try: #for api error
        ...
        rtn_id = bam.addEntity(0, data)
        
        logMsg = "Success: added configuration %s (ObjId:%s)" % (configuration, rtn_id)
