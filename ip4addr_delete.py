...
'''
ip4addr_delete -c config1 -i 10.1.1.10
            ...
            if network_ent['id'] == 0: # no entity found
                ...
                if ipaddr_ent['id'] == 0:
                    ...
                    try:
                        ...
                        # --- delete ipaddr ent ---
                        bam.delete(ipaddr_ent['id'])
                        
                        logMsg = "Success: deleted IP address Name:%s Properity:%s" %  (ip_name , str(prop))
                        print_log.go(log_info, logMsg)
                    except ValueError as e:
                        logMsg = "Failure: %s fail to delete IP address" % str(e)
                        print_log.go(log_error, logMsg)
                        BAM.bam_logout(bam)
                        return(1)
