...
'''
generic-update -c config1 -v view1 -z test.corp -t A -r a1 -i 10.10.10.3 --ni=20.20.20.3
generic-update -c config1 -v view1 -z test.corp -t A -i 10.10.10.3 --ni=20.20.20.3 -k
...
def generic_update(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:z:t:r:i:n:hk",["ni=","debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:z:t:r:i:n:hk",["ni=","debug=","version"])
    ...
    if (rrtype == None) or (rrtype[0:1] == "-"):  
        logMsg = "-t rrtype option is required"
        print_log.go(log_error, logMsg)
        return(1)
    else:
        rrtype = rrtype.upper()
                    ...
                    if convertRR:
                        ...
                        if not (rrtype == None):
                            if rrtype == "NS":
                                if not (rdata == None):
                                    rdata = rdata.lower()
                                if not (newrdata == None):
                                    newrdata = newrdata.lower()
                    ...
                    if len(result_list) == 0: # no entity found
                        ...
                        for ent in result_list:
                            ...
                            if properties['type'] == rrtype:
                                if properties['rdata'] == rdata:  # found match rdata. replaced by newrdata
                                    ...
                                    if debug_api:                        
                                        print ( '------ Entity to be update by bam.update() -------')
                                        print (ent)
                                        # output
