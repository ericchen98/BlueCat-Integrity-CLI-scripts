...
def view_add(*args):    
    ...
    try:
        if configuration:
            ...
            if view:
                ...
                data['name'] = view
                try:
                    rtn_id = bam.addEntity(confid, data)
                except ValueError as e:
                    logMsg =  "Failure: %s Failed to add view: %s" % (str(e), view)
                    print_log.go(log_error, logMsg)
                    BAM.bam_logout(bam)
                    return(1)
                else:
                    logMsg =  "Success: added View %s" % (view)
    ...
    except ValueError as e:     # except for any api error
        logMsg = "%s" % ( str(e) )
        print_log.go(log_error, logMsg)
        BAM.bam_logout(bam) 
        return(1)
