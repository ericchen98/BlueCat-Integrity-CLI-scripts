...
#
# program name: generic-delete 
#
# by: Eric Chen (hchen@bluecatnetworks.com)
# python ver: python 3
# BAM ver: 26.1
#
# note: host is not a generic record. use host-delete.py or host-delete-all.py to delete host record
#
...
# v2.2 : 2020/5/28 correct help by Eden
# v2.1 : 2020/5/26 read config file from dir: /opt/bluecat/cli
# v2.0 : 2020/5/6 add log function.
#         does not put query result in log as there might be multiple  different of records
#         e.g. a1.test.corp type=A, a1.test.corp type=NS etc.
# v1.1 : add -k support same as zone
#

'''
generic-delete -c config1 -v view1 -z test.corp -t a -r pc1 -i 10.10.10.3
generic-delete -c config1 -v view1 -z test.corp -t a -i 10.10.10.3 -k

    -c config name
    -v view name
    -z zone name
    -t record type (ns, a, txt)
    -r record name
...
def generic_delete(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:z:t:r:i:kh", ["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:z:t:r:i:kh", ["debug=","version"])
    ...
    if (rdata == None) or (rdata[0:1] == "-"):  
        logMsg = "Failure: -i rdata option is required"
        print_log.go(log_error, logMsg)
        return(1)


    if (not rrtype) or (rrtype[0:1] == "-"):
        logMsg = "Failure: -t rrtype option is required"
        print_log.go(log_error, logMsg)
        return(1)
    else:
        rrtype = rrtype.upper()
        ...
        if configuration:
            ...
            if view:
                ...
                if zone:
                    ...
                    # getEntitiesByName() only find one record. need to add getAllEntitiesByName()
                    try:
                    ...
                    if len(result_list) == 0: # no entity found
                        ...
                        for ent in result_list:
                            ...
                            if properties['type']==rrtype:
                                #if debug:
                                #   print (properties)

                                if properties['rdata'] == rdata:
                                
                                    print ( "(*) Record:%s, rdata:%s -> deleting..." % (properties['absoluteName'], 
                                               properties['rdata'] ) )
                                    try:
                                        rtn = bam.delete( ent['id'] )
                                        record_changed = True
                                        #logMsg =  "Success: deleted%srecord %s" % ( sameAsZone_Message, 
                                        #        absolutename)
                                        #print_log.go(log_info, logMsg)
                                    except ValueError as e:
                                        logMsg = "Failure: %s Error while deleting record" % str(e)
                                        print_log.go(log_error, logMsg)
                                        BAM.bam_logout(bam)
                                        return(1)
                                else:   #of if properties['rdata'] == rdata:
                                    # print next record until no record to print
                                    print ("    Record:%s, rdata:%s" % (properties['absoluteName'], 
                                                properties['rdata']) )
                        #endof for ent in result_list:
                        
                        # to log only ( connect all IP together ) by print_log.log_only()
                        if not record_changed:
                            logMsg = "Failure: no match%srecord found %s" % (sameAsZone_Message, absolutename)
                            print_log.go(log_error, logMsg)
                            BAM.bam_logout(bam)
                            return(1)
                        else:
                            logMsg = "Success:%srecord deleted %s" % (sameAsZone_Message, absolutename)
                            print_log.go(log_info, logMsg)
                # endof if zone:
            # endof if view:
