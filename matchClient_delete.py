...
# v2.2 2020/5/6 add log function
...
"""
matchClient-delete -c config1 -v view1

    -c config_name
    -v view_name

    Note: will delete whole "match client" option (all ACL items will be deleted)
"""
    ...
    try:
        if configuration:
            ...
            if view:
                ...
                try:
                    # 1) get matchclient
                    # 2) if exist, delete it no matter what it is
                    # 3) if not, printe error msg
                ...
                # oldMatchClientsOption['value'] == '' (this is in case we want to build a delete one item in matchclient script)
                
                if (oldMatchClientsOption['value'] == None) or (oldMatchClientsOption['value'] == '') : 
                    logMsg =  "Failure: matchclient does not exist" 
                    print_log.go(log_error, logMsg)
                    BAM.bam_logout(bam)
                    return(1)
                    
                else:   #of oldMatchClientsOption['value'] == None: (match client exists)
                
                    MatchClient_value = oldMatchClientsOption['value']
                    try:
                        bam.deleteDNSDeploymentOption(viewid, "match-clients","0")    
                    except ValueError as e:
                        logMsg = "Failure: %s Failed to delete match client option" % ( str(e) )
                        print_log.go(log_error, logMsg)
                        BAM.bam_logout(bam)
                        return(1)
                    logMsg =  "Success: delete existing matchclient option: %s" % ( MatchClient_value )
                    print_log.go(log_info, logMsg)
                    BAM.bam_logout(bam)
                    return(0)
                    #endof if oldMatchClientsOption['value'] == None: 
                # endof if not rtn.ok:     # if status_code less than 200
