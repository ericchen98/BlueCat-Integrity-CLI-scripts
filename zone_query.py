...
# v5.6-2023-0827 replace sys.exit() to return() (to work with cli.py)
# v5.5-20221220
#       1) modified: logPath = "./"
...
# v5.3-20211216 remove -l, change flag "-s" to "-a" (all), add deploy flag in print
...
#
'''
zone-query -c config1 -v view1              # list all sub-zones of config1/view1 
zone-query -c config1 -v view1 -z zone1     # query zone1 of config1/view1 
zone-query -c config1 -v view1 -z zone1 -a  # list all sub-zones of config1/view1/zone1 

    -c config_name
    -v view_name
    -z zone_name
    -a                  # list all zones of config/view/zone 
'''
...
def zone_query(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "hdc:v:az:", ["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "hdc:v:az:", ["debug=","version"])
    ...
    if (not zone) or (zone[0:1] == "-"):
        ...
        '''
        if allzone == False:
            logMsg = "Failure: -l option is required"
            print_log.go(log_error, logMsg)
            return(1)
        '''
    ...
    try:
        ...
        if configuration:
            ...
            if view:
                ...
                if allzone:
                    rtn_ent_list = bam.getEntities(viewid, "Zone", 0, max_obj_count)
                    ...
                    for zoneEnt in rtn_ent_list:
                        # print "Zone id: " , ZoneEnt['id'] , " Zone name: " , ZoneEnt['name']
                        
                        properties = bam.splitProp(zoneEnt)
                        deploy_output = ""
                        if properties['deployable'] == 'true':
                            deploy_output = "(deployable=True)"
                        else:
                            deploy_output = "(deployable=False)"
                        
                        print ("Zone name: " + zoneEnt['name'] + ' ' + deploy_output)
                        RecordPrintCount +=  + 1
                        if RecordPrintCount == 1:
                            show_list = show_list + zoneEnt['name'] 
                        else:
                            show_list =  show_list + ',' + zoneEnt['name']
                    ...
                    if RecordPrintCount==0:
                        ...
                        logMsg = "Success: success found zone: %s" % (show_list)
                        print_log.go(log_info, logMsg)
                        BAM.bam_logout(bam)
                        return(0)
                else:   # of if allzone: (search all subzones in a zone)
                    if subzone:
                        zoneEnt = bam.find_zone(viewid, zone)
                        
                        parentZoneid = zoneEnt['id']
                        if parentZoneid == 0:
                        ...
                        SubZoneEnt = bam.getEntities(parentZoneid, "Zone", 0, max_obj_count)
                        RecordPrintCount=0
                        # for print all sub zone info only (not used anymore)
                        #show_list = ""
                        
                        for workingEnt in SubZoneEnt:
                        
                            properties = bam.splitProp(workingEnt)
                            deploy_output = ""
                            if properties['deployable'] == 'true':
                                deploy_output = "(deployable=True)"
                            else:
                            ...
                            print ( f"zone name: {workingEnt['name']}.{zone} {deploy_output}" )   
                            RecordPrintCount = RecordPrintCount+1
                            
                            # Since the zone name could be long and many, we don't log it in log file
                            #if RecordPrintCount == 1:
                            #    show_list = show_list + workingEnt['name']
                            #else:
                            #    show_list =  show_list + ',' + workingEnt['name']
                        ...
                        if RecordPrintCount==0:
                            ...
                            #logMsg = "Success: success to get zone: %s" % (show_list)
                            print
                            logMsg = "Success: success found sub-zone of zone: %s" % zone
                            print_log.go(log_info, logMsg)
                            BAM.bam_logout(bam)
                            return(0)
                    else: #of if subzone: query one zone only
                        zoneEnt = bam.find_zone(viewid, zone)
                        zoneId = zoneEnt['id']
                        

                        if zoneId == 0:   # not found
                            logMsg = "Failure: Failed to get zone: %s" % zone
                            print_log.go(log_error, logMsg)
                            BAM.bam_logout(bam)
                            return(1)
                            ...
                            print
                            logMsg = "Success: success found zone: %s %s" % (zone, deploy_output)
                            print_log.go(log_info, logMsg)
                            BAM.bam_logout(bam)
                            return(0)
                    #endof if subzone:
