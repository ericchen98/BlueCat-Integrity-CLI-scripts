...
#
'''
macPoolItem-add -c config1 -p macPool1 -m 1122334455

    -c config_name
    -p macPool      (use "deny" to add/delete MAC to "Deny MAC Pool")
    -m MacAddress   (Format can be "nnnnnnnnnnnn" or nn-nn-nn-nn-nn-nn" or nn:nn:nn:nn:nn:nn")

    # add MAC address 112233445566 to MAC Pool "macPool1"
'''
                ...
                if not deny_macPool:
                    ...
                    if macPoolid == 0:
                        ...
                        if macid == 0:
                            ...
                            rtn = bam.associateMACAddressWithPool(confid, mac, macPoolid)
                            
                            logMsg =  "Success: add MAC address:%s to MACPool:%s " % (mac, macPool)
                        ...
                        if macid == 0:
                            ...
                            logMsg =  "Success: add MAC address:%s to MACPool:DENY" % (mac)
                            print_log.go(log_info, logMsg)
