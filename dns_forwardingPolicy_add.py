...
# v1.1-20260712 create


"""
dns_forwardingPolicy_add -c config1 -v view1 -o
    -c config_name
    -v view_name
    -s dds_server 
    -g server_group
    -y                 # add option to server or serverGroup instead of configuration
    -o                 # add "only" as the forwarding policy
    -f                 # add "first" as forwarding policy
    
dns_forwardingPolicy_add -c config1                     -o # add forwardingPolicy=only to config1
dns_forwardingPolicy_add -c config1 -v view1            -f # add forwardingPolicy=first to view1 in config1
dns_forwardingPolicy_add -c config1 -v view1 -s dds1       # add forwardingPolicy=first to dds1 in view1
dns_forwardingPolicy_add -c config1 -v view1 -g sg1     -o # add forwardingPolicy=only to serverGroup sg1 in view1
dns_forwardingPolicy_add -c config1          -s dds1    -o # add forwardingPolicy=only to dds1 in config1
dns_forwardingPolicy_add -c config1          -g sg1     -o # add forwardingPolicy=only to serverGroup sg1 in config1
dns_forwardingPolicy_add -c config1          -s dds1 -y -o # add forwardingPolicy=only to server dds1
dns_forwardingPolicy_add -c config1          -g sg1  -y -o # add forwardingPolicy=only to serverGroup sg1
"""
...
# main function
# ---------------------------------------------------------------------
def dns_forwardingPolicy_add(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:s:g:foyh", ["debug=","version"])
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:s:g:foyh", ["debug=","version"])
    ...
    for o,v in opts:
        ...
        elif o == "-f": value_first = True
        elif o == "-o": value_only = True
    ...
    # the way to set policy value
    # prop_config = 'FIRST' or 'ONLY'


    if value_first and value_only:
        logMsg = "Failure: -f and -o cannot exist at the same time"
        print_log.go(log_error, logMsg)
        return (1)
    elif value_first:
        prop_config = 'FIRST'
        ...
        prop_config = 'FIRST' #default value if -f -o are not set
    ...
    # note: python dns_forwarding_add-t1.py --debug api -c config1 -v  -s srv1
    #
    # [('--debug', 'api'), ('-c', 'config1'), ('-v', '-s')] <--- consider treat "-*" as illegial input
    ...
    try:
        ...
        if do_config_go: # add option to config
            # -----------------------------------------------------------------
            #  add option to config Note: prop_config='FIRST' or 'ONLY'
            # -----------------------------------------------------------------
            try:
                ent = bam.addDNSDeploymentOption(confid, "forwarding-policy", prop_config, '')
                ...
                logMsg =  'Success: add DNS forwarding-policy option:"%s" to config:"%s"' % ( 
            ...
            # -----------------------------------------------------------------
            #  add option to server under view(do_view=T, do_server=T)
            # -----------------------------------------------------------------
            if view:
                ...
                for ent in result_list:
                    ...
                    if server.upper() ==  server_name.upper(): # --- found target server to add
                        ...
                        try:
                            rtn_id = bam.addDNSDeploymentOption( viewid, "forwarding-policy", prop_config , 
                                     prop_add_to_server)
                            ...
                            logMsg = 'Success: add DNS forwarding option:"%s" to server:"%s" in view: "%s"' % (
                                     prop_config, server, view)
                            print_log.go(log_info, logMsg)
            ...
            # -----------------------------------------------------------------
            #  add option to serverGroup in view
            # -----------------------------------------------------------------
            ...
            if view:
                ...
                for ent in result_list:
                    ...
                    if serverGroup.upper() ==  serverGroup_name.upper(): # --- found target server to add
                        ...
                        # Note: when properity="serverGroup=209051" (server=server_id) will add option to server under view
                        # note2: if properity="server=0". Will output error (invalid Invalid server id or server group id: 0") 
                        ...
                        try:
                            ...
                            logMsg =  'Success: add DNS forwarding-policy option:"%s" to serverGroup:"%s" in view:"%s"' % ( prop_config, serverGroup, view)
                            print_log.go(log_info, logMsg)
            ...
            try:
                ...
                logMsg =  'Success: add DNS forwarding-policy option:"%s" to view:"%s" ' % (prop_config, view) 
            ...
            for ent in result_list:
                ...
                if server.upper() ==  server_name.upper(): # --- found target server to add
                    ...
                    try:
                        rtn_id = bam.addDNSDeploymentOption(confid, "forwarding-policy", prop_config , prop_add_to_server)
                        ...
                        logMsg =  'Success: add DNS forwarding-policy option:"%s" to server:"%s" in config:"%s"' % ( 
                                                            prop_config, server, configuration )
                        print_log.go(log_info, logMsg)
            ...
            # -----------------------------------------------------------------
            #  add option [to serverGroup only]
            # -----------------------------------------------------------------
            ...
            for ent in result_list:
                ...
                if serverGroup.upper() ==  serverGroup_name.upper(): # --- found target server to add
                    ...
                    try:
                        ...
                        logMsg =  'Success: add DNS forwarding-policy option:"%s" to serverGroup:"%s" in config:"%s"' % ( 
                                   prop_config, serverGroup, configuration )
            ...
            # ------------- to find server, config name need to be provided!!!
            ...
            for ent in result_list:
                ...
                if server.upper() ==  server_name.upper(): # --- found target server to add
                    ...
                    #--- add deploy option to server
                    try:
                        rtn_id = bam.addDNSDeploymentOption(server_id, "forwarding-policy", prop_config , '')
                        ...
                        logMsg =  'Success: add DNS forwarding-policy option:"%s" to server:"%s"' % ( 
                                                            prop_config, server )
                        print_log.go(log_info, logMsg)
            ...
            for ent in result_list:
                ...
                if serverGroup.upper() ==  serverGroup_name.upper(): # --- found target server to add
                    ...
                    try:
                        rtn_id = bam.addDNSDeploymentOption(serverGroup_id, "forwarding-policy", prop_config , prop_add_to_server)
                        ...
                        logMsg =  'Success: add DNS forwarding-policy option:"%s" to serverGroup:"%s"' % ( 
                                  prop_config, serverGroup)
                        print_log.go(log_info, logMsg)
