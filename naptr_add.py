...
# v5.5-20220414 per request, check input field: order & preference range in 0~65535 
# v5.4-20220221 update BAM.bam_logout(bam) before all return()
# v5.3 add input debug
#      - remove (') in help
...
#
# ----------------------------- INPORTANT NOTE --------------------------
# when running this script under bash, -e need to be quoted: e.g. -e '!^.*$!sip:user@sip.rfc1035.com!'
...
'''
naptr-add -c config1 -v view1 -z test.corp -r naptr1 -g S -o 100 -p 10 -e !^.*$!sip:user@sip.rfc1035.com! -t test1.MNC001.MCC700.gprs. -s x-3gpp-pgw:x-gp:x-gn
    ...
    -t replacement
    -s service
    -k same as zome record (when -k is used, -r is not required)
'''
import getopt
import sys
import json
...
def naptr_add(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "hdc:v:z:r:o:p:s:e:t:g:k", ["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "hdc:v:z:r:o:p:s:e:t:g:k", ["debug=","version"])
            ...
            if view:
                ...
                if zone:
                    ...
                    try:
                        ...
                        rtn_id = bam.addNAPTRRecord ( viewid,  add_absolutename,  order,  preference, service, \
                            regexp,  replacement,  flags,  ttl,  properties )
                        logMsg =  "Success: Added%srecord %s" % ( sameAsZone_Message, absolutename)
