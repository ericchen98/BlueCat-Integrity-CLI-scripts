...
#
'''
zone-update -c config1 -v view1 -z zone1 -y  # set deployable to True for config1/view1/zone1 
zone-update -c config1 -v view1 -z zone1 -n  # set deployable to False for config1/view1/zone1  
    
    -c config_name
    -v view_name
    -z zone_name
    -y                      # set deployable to True  to zone_name under config_name/view_name 
    -n                      # set deployable to False to zone_name under config_name/view_name 
'''
...
def zone_update(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "hdc:v:z:yn", ["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "hdc:v:z:yYnN", ["debug=","version"])
    ...
    for o,v in opts:
        ...
        elif o == "-Y": deploy = "True"
        elif o == "-y": deploy = "True"
        elif o == "-N": deploy = "False"
        elif o == "-n": deploy = "False"
    ...
    if deploy == '':
        logMsg = "Failure: -y or -n is required"
        print_log.go(log_error, logMsg)
        return(1)
                    ...
                    if zoneId == 0:   # not found
                        ...
                        if properties['deployable'] == 'true':
                            if deployable == True:
                                ...
                                logMsg = "Success: zone: %s is already deployable=True" % (zone)
                                print_log.go(log_info, logMsg)
                                BAM.bam_logout(bam)
                                return(0)
                            else: # deployable == False:
                                # set deployable to false 
                                properties['deployable'] = 'false'
                                zoneEnt['properties'] = bam.joinProp(properties)
                                
                                try:
                                    rtn = bam.update (zoneEnt)
                                    
                                except ValueError as e:
                                    logMsg = "Failure: %s Failed to update zone: %s" % (str(e), zone)
                                    print_log.go(log_error, logMsg)
                                    BAM.bam_logout(bam)
                                    return(1)
                                else:    
                                    logMsg =  "Success: Update zone %s with deployable=%s" % (zone, deployable)
                                    print_log.go(log_info, logMsg)
                        else: #if properties['deployable'] == 'true':
                            if deployable == False:
                                # same value, do nothing
                                logMsg = "Success: zone: %s is already deployable=False" % (zone)
                                print_log.go(log_info, logMsg)
                                BAM.bam_logout(bam)
                                return(0)
                            else: # deployable == True:
                                # set deployable to false 
