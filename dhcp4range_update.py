...
#
# program name: dhcp4range_update.py
# by: Eric Chen (hchen@bluecatnetworks.com)
# python ver: python 3
# BAM ver: 26.1
#
# v1.0-20260918 created (same style as dhcp4range_add.py/dhcp4range_query.py)
#


'''
dhcp4range_update -c config1 -i 10.1.1.0/24 -s 10.1.1.11 -e 10.1.1.20 --ns 10.1.1.15 --ne 10.1.1.25
dhcp4range_update -c config1 -i 10.1.1.0/24 -s 10.1.1.15 -e 10.1.1.25 -n new_name

     -c config1                 # config name e.g. config1
     -i ip4network/subnet       # ipv4 network e.g. 10.1.1.0/24
     -s start_ip                # current start ip e.g. 10.1.1.11
     -e end_ip                  # current end ip e.g. 10.1.1.20
     -n name                    # (optional) new name for the range
     --ns new_start_ip          # (optional) new start ip e.g. 10.1.1.15
     --ne new_end_ip            # (optional) new end ip e.g. 10.1.1.25

     # update (resize/move/rename) an existing dhcpv4 range within a
     # network - provide -n, or both --ns and --ne, or both at once
'''
...
def dhcp4range_update(*args):
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:i:s:e:n:h",["ns=","ne=","debug=","version"])
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:i:s:e:n:h",["ns=","ne=","debug=","version"])
    ...
    for o,v in opts:
        ...
        elif o == "--ns": new_start_ip = v
        elif o == "--ne": new_end_ip = v
    ...
    if (not end_ip) or (end_ip[0:1] == "-"):
        ...
        print_log.go(log_error, logMsg)
        return(1)
    # --ns/--ne must be given together (a range needs both ends), but the
    # whole pair is optional as long as -n (rename) is given instead - and
    # both a rename and a range change can be requested at the same time.
    if (new_start_ip is not None) != (new_end_ip is not None):
        logMsg = "Failure: --ns and --ne must be given together"
        print_log.go(log_error, logMsg)
        return(1)
    if (new_start_ip is not None) and (new_start_ip[0:1] == "-"):
        logMsg = "Failure: --ns new_start_ip option is invalid"
        print_log.go(log_error, logMsg)
        return(1)
    if (new_end_ip is not None) and (new_end_ip[0:1] == "-"):
        logMsg = "Failure: --ne new_end_ip option is invalid"
        print_log.go(log_error, logMsg)
        return(1)
    if (name is not None) and (name[0:1] == "-"):
        logMsg = "Failure: -n name option is invalid"
        print_log.go(log_error, logMsg)
        return(1)
    if (new_start_ip is None) and (name is None):
        logMsg = "Failure: provide -n, or both --ns and --ne (or both)"
        print_log.go(log_error, logMsg)
        return(1)
            ...
            if confid == 0:
                ...
                if len(result_list) == 0: # no entity found
                    ...
                    if range_ent['id'] == 0: # no entity found
                        ...
                        properties = bam.splitProp(range_ent)

                        change_desc = []

                        if new_start_ip is not None:
                            # v2's real field is a single "range" string
                            # "<start> - <end>" (confirmed live via
                            # debug_dhcp4range.py) - set it directly using
                            # the real v2 field name so it passes through
                            # bam.update() unchanged (see BAM.py's
                            # _v1_props_for_write()/UPDATE_SKIP_PROPS).
                            properties['range'] = "%s - %s" % (new_start_ip, new_end_ip)
                            change_desc.append("Range: %s-%s -> %s-%s" % (
                                    start_ip, end_ip, new_start_ip, new_end_ip))

                        range_ent['properties'] = bam.joinProp(properties)

                        if name is not None:
                            # bam.update() applies a non-None top-level
                            # 'name' on the entity dict directly (see
                            # BAM.py's update()), same mechanism a_update.py
                            # style scripts use to rename an entity.
                            old_name = range_ent.get('name') or ''
                            range_ent['name'] = name
                            change_desc.append("Name: %s -> %s" % (old_name, name))
                        ...
                        try:
                            bam.update(range_ent)

                            logMsg = "Success: updated dhcp v4 Range. Network: %s. %s" % (
                                    ipNetwork, "; ".join(change_desc))
                            print_log.go(log_info, logMsg )
                            record_changed = True

                        except ValueError as e:
                            logMsg = "Failure: %s Failed to update dhcp v4 range" % str(e)
                            print_log.go(log_error, logMsg)
                            error_Logged = True

                        if not record_changed and not error_Logged:
                            logMsg = "Failure: failed to update dhcp v4 range"
                            print_log.go(log_error, logMsg)
