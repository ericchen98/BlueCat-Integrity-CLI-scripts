...
# BAM ver: 26.1
#
# v1.1-20230828 fixed log does not show in cli.py issue 
# v1.0-20221220
#
'''
macPoolItem-delete -c config1 -p macPool1 -m 1122334455

    -c config_name
    -p macPool
    -m MacAddress   (Format can be "nnnnnnnnnnnn" or nn-nn-nn-nn-nn-nn" or nn:nn:nn:nn:nn:nn")

    # delete MAC address "112233445566" from MAC Pool "macPool1"
...
def macPoolItem_delete(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:p:m:h", ["debug=","version"])           
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:p:m:h", ["debug=","version"])
    ...
    if (not macPool) or (macPool[0:1] == "-"):
        logMsg = "Failure: -p macPool option is required"
        print_log.go(log_error, logMsg)
        return(1)

    if macPool.upper() == "DENY":
        deny_macPool = True

    if (not mac) or (mac[0:1] == "-"):
        logMsg = "Failure: -m mac option is required"
        print_log.go(log_error, logMsg)
        return(1)
    ...
    try:
        if configuration:
            ...
            if macPool:
                ...
                if not deny_macPool:
                    macPool_ent = bam.getEntityByName(confid, macPool, "MACPool")
                    macPoolid = macPool_ent['id']

                    if macPoolid == 0:
                        logMsg = "Failure: MacPool %s does not exist" % (macPool)
                        print_log.go(log_error, logMsg)
                        BAM.bam_logout(bam)
                        return(1)
                        #{'id': 130132, 'name': 'p01', 'type': 'MACPool', 'properties': 'instantDeploy=true|'}

                    else:
                        #getMACAddress
                        #mac_ent = bam.getEntityByName(confid, mac, "MACAddress")
                        
                        mac_ent = bam.getMACAddress(confid, mac)
                        macid = mac_ent['id']
                        
                        if macid == 0:
                            logMsg = "Failure: MAC address %s does not exist" % (macPool)
                            print_log.go(log_error, logMsg)
                            BAM.bam_logout(bam)
                            return(1)
                        else:
                            # macPool and mac are all exist, link them.
                            
                            if debug_api:
                                print('---[debug] print MACAddress entity ---')
                                print(mac_ent)
                                
                                # output:
                                # {'id': 100917, 'name': None, 'type': 'MACAddress', 
                                #  'properties': 'address=11-22-33-44-55-66|macPool=p01|macVendor=Not Found|'}
                            
                            rtn = bam.unlinkEntities(macPoolid, macid )
                            #remove macPool from mac address
                             
                            logMsg =  "Success: remove MAC address:%s from MACPool:%s " % (mac, macPool)
                            print_log.go(log_info, logMsg)
                    
                else:  # of if deny_macPool:
                        mac_ent = bam.getMACAddress(confid, mac)
                        macid = mac_ent['id']
                        ...
                        if macid == 0:
                            ...
                            #how to unlink a Mac Addr from deny mac pool?
                            #rtn = bam.denyMACAddress(confid, mac)
                            
                            
                            #get Deny MAC Pool entity
                            denyMacPool_ent = bam.getEntityByName(confid, 'deny', "DenyMACPool")
                            

                            # print(denyMacPool_ent)
                            #output:
                            #{'id': 100884, 'name': 'DENY', 'type': 'DenyMACPool', 'properties': None}
                                                      
                            
                            
                            denyMacPool_id = denyMacPool_ent['id']
                            
                            rtn = bam.unlinkEntities(denyMacPool_id, macid )
                            
                            logMsg =  "Success: deleted MAC address:%s from MACPool:DENY " % (mac)
                            print_log.go(log_info, logMsg)
