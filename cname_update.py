...
#
# program name: cname-update.py
# by: Eric Chen (hchen@bluecatnetworks.com)
# python ver: python 3
# BAM ver: 26.1
#
# v1.0-20260918 (based on a_update.py's style)

'''
cname-update -c config1 -v view1 -z test.corp -r c3 --ni h2.test.corp

    -c config_name
    -v view_name
    -z zone_name
    -r record_name (cname record name)
    --ni= new linkedRecordName (the new target host record)

    # update a cname record's link target in a zone
    # note: a cname record's name is unique within a zone (unlike a host
    # record, which can have multiple addresses), so there is no -i
    # old-value flag here - the record is found by name and its target is
    # simply replaced with --ni.
'''
...
def cname_update(*args):
    ...
    #----------------------------------------------------------------
    #get program name. If prog is called by cli.py, then __name__ will be "cname_update"
    prog_name = __name__

    #convert "_" to "-" in prog_name
    prog_name = prog_name.replace('_','-')

    if __name__ == '__main__': #this program is run by python directly (e.g. python3 cname_update.py)
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:z:r:h",["ni=","debug=","version"])
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:z:r:h",["ni=","debug=","version"])
    ...
    if (newlinkedRecordName == None) or (newlinkedRecordName[0:1] == "-"):
        logMsg = "--ni newlinkedRecordName option is required"
        print_log.go(log_error, logMsg)
        return(1)
            ...
            if view:
                ...
                if zone:
                    ...
                    if len(result_list) == 0: # no entity found
                        ...
                        print_log.go(log_error, logMsg)
                    else: # found entity - a cname's name is unique in a zone, so use the first match
                        ent = result_list[0]
                        ...
                        if properties['type']=="CNAME":

                            prnMsg = '(*) Record: %s, linkedRecordName: %s -> is updating to linkedRecordName: %s' % (
                                properties['absoluteName'], properties['linkedRecordName'], newlinkedRecordName )
                            print (prnMsg)
                            ...
                            try:
                                ...
                                prnMsg = "Success: Updated record %s linkedRecordName from %s to %s" % (
                                     absolutename, properties['linkedRecordName'], newlinkedRecordName)
                                print (prnMsg)
                                record_changed = True
                                ...
                                record_changed = False
                                error_Logged = True
                                # cannot run return(1) as we want to print the final message

                        # ----- print final success/Failure message ----------

                        if record_changed:
                            print ()
                            logMsg =  "Success: Updated record %s linkedRecordName to %s" % (
                                                                    absolutename, newlinkedRecordName)
                            ...
                            if not error_Logged:
                                print ()
                                logMsg = "Failure: failed to update record %s" % (absolutename)
                                print_log.go(log_error, logMsg)
                                BAM.bam_logout(bam)
                                return(1)
