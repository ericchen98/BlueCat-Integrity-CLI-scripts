...
# 
# program name: view-add.py (changed from add-confzone.py)
#
...
# v5.3 20211216 fix deploy=true not add (need join prop) : done. 
#      20220221 Checked, deploy role won't add to parent
# v5.1 20210921 (use BAM.py v20210918-1735)
# v5.0 20210915-1500
#
'''
zone-add -c config1 -v view1 -z zone        # create zone1 without deplyable flag
zone-add -c config1 -v view1 -z zone -y     # create zone1 with deplyable flag set
    -c config_name
    -v view_name
    -z zone_name
    -y              # set deployable flag
'''
...
# remark this to display error and line number
...
def zone_add(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:z:yh", ["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:z:yh", ["debug=","version"])
        ...
        if configuration:
            ...
            if view:
                ...
                if zone:
                    absoluteName = zone     
                    # prop is
                    # {'deployable': 'true', 'absoluteName': 'test1.corp2'}
                    
                    prop={}
                    if deploy == True:
                        prop['deployable'] = 'true'
                    else:
                        prop['deployable'] = 'false'
                    prop['absoluteName'] = zone
                    prop_to_go = bam.joinProp(prop)
                    
                    try:
                        rtn = bam.addZone(viewid, absoluteName, prop_to_go)
                    except ValueError as e:
                        logMsg = "Failure: %s Failed to add zone: %s" % (str(e), zone)
                        print_log.go(log_error, logMsg)
                        BAM.bam_logout(bam)
                        return(1)
                    else:    
                        logMsg =  "Success: added zone %s (objId:%s)" % (zone, rtn)
