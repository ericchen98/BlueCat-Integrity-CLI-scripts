...
"""
matchClient-add -c config1 -v view1 -m 10.1.1.0/24,10.1.2.0/24

    -c config_name
    -v view_name
    -m match clint IP and subnet list (use "," to seperate each item)

    Note: Important: if an invalid ACL is added, it will cause BDDS deploy fail in BAM GUI.
"""
        ...
        if configuration:
            ...
            if view:
                ...
                if viewid == 0:
                    ...
                    return(1)
                # ------------------------------------------------ 
                # End of read config/view
                # ------------------------------------------------ 
                try:
                    rtn_id = bam.addDNSDeploymentOption(viewid, "match-clients", matchclient , "")
                except ValueError as e:
                        logMsg = "Failure: %s Failed add match client option %s" % ( str(e),matchclient )
                        print_log.go(log_error, logMsg)
                        BAM.bam_logout(bam)
                        return(1)
                else:    
                    logMsg =  "Success: Added matchclient option with value '%s'" % ( matchclient )                
