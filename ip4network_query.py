...
#
# program name: ip4network-query.py
# by: Eric Chen (hchen@bluecatnetworks.com)
# python ver: python 3
# BAM ver: 26.1
#
# v1.1-20251228 modify remark
# v1.0-20250922 created
#

'''
ip4network_query -c config1 -i 10.1.1.0/24 -n new_name -u UDF1 -v UDF_value -y
...
def ip4network_query(*args): 
    ...
    #            getopt.getopt() : will ignore items after '' ('--dry' will be ignored in this example)
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:i:",["ni=","debug=","dry", "version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:i:",["ni=","debug=","dry", "version"])
            ...
            if confid == 0:
               ...
               # ent found, but need to tell whether it is in same config!!!
                ...
                if len(result_list) == 0: # no entity found
                    ...
                    allowDuplicateHost = prop['allowDuplicateHost']
                    pingBeforeAssign = prop['pingBeforeAssign']
                    inheritAllowDuplicateHost = prop['inheritAllowDuplicateHost']
                    inheritDefaultDomains = prop['inheritDefaultDomains']
                    inheritDefaultView = prop['inheritDefaultView']
                    
                    gateway = prop['gateway']
                    dynamicUpdate = prop['dynamicUpdate']
