...
#          Note: For rdata, only "NS" will be convert. (NOt convert RRtype: A, TXT etc)
...
'''
generic-add -c config1 -v view1 -z test.corp -r pc1 -i 10.10.10.3   -t A
generic-add -c config1 -v view1 -z test.corp -r pc1 -i a1.test.corp -t NS
generic-add -c config1 -v view1 -z test.corp -i 10.10.10.3 -k       -t A
...
def generic_add(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:z:r:ki:t:h", ["debug=","version"])           
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:z:r:ki:t:h", ["debug=","version"])
            ...
            if view:
                ...
                if zone:
                    ...
                    try:
                        ...
                        ent_id = bam.addGenericRecord( viewid, add_absolutename, rrtype, rdata,ttl ,properties)
                        
                        logMsg = f"Success: added{sameAsZone_Message}record {absolutename} Rdata:{rdata}"
