...
# v5.5-20220402 rename host-update.py to host-update-one.py to meet backward compatibility 
# v5.4-20220221 update BAM.bam_logout(bam) before all return()
# v5.3 2021-1010 use --i instead of --ni
# v5.1-20210921 (use BAM.py v20210918-1735)
# v5.0 20210915-1500
# v4.0 2021/8/21 use new BAM that work with 9.3 & REST
...
'''
host-update-one -c config1 -v view1 -z test.corp -r host1 -i 10.10.10.3 --ni 10.10.10.4 
host-update-one -c config1 -v view1 -z test.corp -i 10.10.10.3 --ni 10.10.10.4 -k 
# update same as zone record
host-update-one -c config1 -v view1 -z test1.corp -r h1 -i 10.1.1.2 --ni 10.1.1.2 -p yes 
# only modify ptr to yes
    ...
    -r  record_name
    -i  rdata
    --ni new_rdata_to_update
    -p  [yes|no]     ('yes':add ptr. 'No':remove ptr. if -p is not set, will keep the original setting)
    -k  (same as zome record. when -k is used, -r is ignored)

    # Update an IP of a host record
'''
import getopt
import sys
import json
...
def host_update_one(*args):    
    ...
    for o,v in opts:
        ...
        elif o == "-p": addptr_yn = v       # inputing 'yes', 'true', 'no', 'false' are all supported
    ...
    if not newrdata: 
        if (newrdata == None) or (newrdata[0:1] == "-"):
            logMsg = 'Failure: --ni rdata option is required'
            print_log.go(log_error, logMsg)
            return(1)
            
    else: #newrdata exists
        try:
            # this is to test whether the rdata is a valid IPv6 (and ipv4) address
            # note: to support ipv6, this script does not support adding multiple address 
            # at the same time anymore (e.g. -i 10.1.1.1,10.2.2.2). To do it, use host-add-all.py
            ...
            # will do exception if IP address format is wrong
            foo1 = ipaddress.ip_address(newrdata)
    ...
    # --- for PTR config ---
    addptr_yn_set = False   # if -p is not set
    if addptr_yn.upper() in ('YES', 'TRUE') :
        addptr = True
        addptr_yn_set = True
    else:
        if addptr_yn.upper() in ('NO', 'FALSE'):
            addptr = False
            addptr_yn_set = True
                    ...
                    if ent['id'] == 0:  # no existing record found. Do nothing, exist
                        ...
                        for index in range(len(addrList)):
                            if ipaddress.ip_address(addrList[index]) == ipaddress.ip_address(rdata):
                                ...
                                # update found IP by newrdata
                                addrList[index] = newrdata
                                ...
                                org_ptr = properties['reverseRecord']
                                ptr_info = org_ptr  # keep the original ptr info

                                if addptr_yn_set == True:
                                    if addptr:  # user input -p yes
                                        properties['reverseRecord'] = 'true'
                                        ptr_info = 'true'
                                    else:       # user input -p no
                                ...
                                try:
                                    bam.update (ent)
                                    logMsg =  "Success: Update%srecord %s IP %s to %s PTR=%s" % ( 
                                                sameAsZone_Message, absolutename, rdata, newrdata, ptr_info)
                        ...
                        if ip_match == False:
                            logMsg = "Failure:%shost record %s found, but IP %s not found" % ( sameAsZone_Message, 
                                        absolutename, rdata)
                            print_log.go(log_error, logMsg)
                        ...
                        # just print the host record info without needing -d (this info should be good to user)
                        
                        print ("\nCurrent%sHost record %s: %s PTR=%s" % ( sameAsZone_Message, 
                                absolutename, properties['addresses'], properties['reverseRecord'] ) )
