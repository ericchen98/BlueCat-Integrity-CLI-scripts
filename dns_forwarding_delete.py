...
dns-forwarding-delete -c config1                     # delete option of config1
dns-forwarding-delete -c config1 -v view1            # delete option of view1 in config1
dns-forwarding-delete -c config1 -v view1 -s dds1    # delete option of view1
dns-forwarding-delete -c config1 -v view1 -g sg1     # delete option of serverGroup sg1 in view1
dns-forwarding-delete -c config1          -s dds1    # delete option of dds1 in config1
dns-forwarding-delete -c config1          -g sg1     # delete option of serverGroup sg1 in config1
dns-forwarding-delete -c config1          -s dds1 -y # delete option of server dds1
dns-forwarding-delete -c config1          -g sg1  -y # delete option of serverGroup sg1
"""
...
python dns_forwarding_add.py    -c config1 -v view1            -i 1.1.1.1,8.8.8.8 -z yes 
python dns_forwarding_delete.py -c config1 -v view1

python dns_forwarding_add.py    -c config1 -v view1 -s dds1    -i 1.1.1.1,8.8.8.8 -z yes 
python dns_forwarding_delete.py -c config1 -v view1 -s dds1

python dns_forwarding_add.py    -c config1 -v view1 -g sg1     -i 1.1.1.1,8.8.8.8 -z yes 
python dns_forwarding_delete.py -c config1 -v view1 -g sg1

python dns_forwarding_add.py    -c config1 -v view1            -i 1.1.1.1,8.8.8.8 -z yes 
python dns_forwarding_delete.py -c config1 -v view1

python dns_forwarding_add.py    -c config1    -s dds1    -i 1.1.1.1,8.8.8.8 -z yes 
python dns_forwarding_delete.py -c config1    -s dds1

python dns_forwarding_add.py    -c config1    -g sg1     -i 1.1.1.1,8.8.8.8 -z yes 
python dns_forwarding_delete.py -c config1    -g sg1

python dns_forwarding_add.py    -c config1    -s dds1 -y -i 1.1.1.1,8.8.8.8 -z yes 
python dns_forwarding_delete.py -c config1    -s dds1 -y

python dns_forwarding_add.py    -c config1    -g sg1 -y -i 1.1.1.1,8.8.8.8 -z yes
python dns_forwarding_delete.py -c config1    -g sg1 -y
'''
...
# main function
# ---------------------------------------------------------------------
def dns_forwarding_delete(*args):    
    ...
    try:
        ...
        if do_config_go: # add option to config
            ...
            if ent['id']== 0:
                ...
                try:
                    bam.deleteDNSDeploymentOption(confid, "forwarding", "0")
                except ValueError as e:
                    ...
                    logMsg =  ('Success: delete DNS forwarding option:"%s" [Disable forwarding for Child zone]:"%s"'
                                'in config:"%s"') % (ip_join, disable_forward_child_zones, configuration )
            ...
            if view:
                ...
                for ent in result_list:
                    ...
                    if server.upper() ==  server_name.upper(): # --- found target server to add
                        try:
                            ent = bam.getDNSDeploymentOption(viewid, "forwarding", server_id)
                            if debug_api:
                                print('[debug: ------- search result of getDNSDeploymentOption() -------]')
                                print(ent)
                        ...
                        if ent['id']== 0:
                            logMsg = ('Failure: DNS forwarding option of server:"%s" in view:"%s"'
                            ...
                            try:
                                bam.deleteDNSDeploymentOption(viewid, "forwarding", server_id)
                                do_success = True
                            except ValueError as e:
                                    logMsg = "Failure: %s Failed to delete DNS forwarding option" % ( str(e) )
                                    print_log.go(log_error, logMsg)
                                ...
                                logMsg =  ('Success: delete DNS forwarding option:"%s" '
                                  '[Disable forwarding for Child zone]:"%s" server:"%s" in view:"%s"') % (
                                  ip_join, disable_forward_child_zones, server, view)
            ...
            if view:
                ...
                for ent in result_list:
                    ...
                    if serverGroup.upper() ==  serverGroup_name.upper(): # --- found target server to add
                        try:
                            ent = bam.getDNSDeploymentOption(viewid, "forwarding", serverGroup_id)
                            if debug_api:
                                print('[debug: ------- search result of getDNSDeploymentOption() -------]')
                                print(ent)
                        ...
                        if ent['id']== 0:
                            logMsg = ('Failure: DNS forwarding option of serverGroup:"%s" '
                            ...
                            try:
                                bam.deleteDNSDeploymentOption(viewid, "forwarding", serverGroup_id)
                                do_success = True
                            except ValueError as e:
                                    logMsg = "Failure: %s Failed to delete DNS forwarding option" % ( str(e) )
                                    print_log.go(log_error, logMsg)
                                ...
                                logMsg =  ('Success: delete DNS forwarding option:"%s" '
                                    '[Disable forwarding for Child zone]:"%s" serverGroup:"%s" in view:"%s"') % (
                                    ip_join, disable_forward_child_zones, serverGroup, view)
                                print_log.go(log_info, logMsg)
                                BAM.bam_logout(bam)
                                return(0)
                        break
            ...
            if viewid == 0:
                ...
                if ent['id']== 0:
                    logMsg = 'Failure: DNS forwarding option of config:"%s" does not exist' % (view)
                    print_log.go(log_error, logMsg)
                    BAM.bam_logout(bam)
                    return(1)
                    ...
                    try:
                        bam.deleteDNSDeploymentOption(viewid, "forwarding", "0")
                    except ValueError as e:
                            logMsg = "Failure: %s Failed to delete DNS forwarding option" % ( str(e) )
                            print_log.go(log_error, logMsg)
                            BAM.bam_logout(bam)
                        ...
                        logMsg =  ('Success: delete DNS forwarding option:"%s" '
                                   '[Disable forwarding for Child zone]:"%s" in view:"%s"') % (
                                  ip_join, disable_forward_child_zones, view )
            ...
            for ent in result_list:
                ...
                if server.upper() ==  server_name.upper(): # --- found target server to add
                    try:
                        ent = bam.getDNSDeploymentOption(confid, "forwarding", server_id)
                        if debug_api:
                            print('[debug: ------- search result of getDNSDeploymentOption() -------]')
                            print(ent)
                    ...
                    if ent['id']== 0:
                        logMsg = ('Failure: DNS forwarding option of server:"%s" in config:"%s" '
                        ...
                        try:
                            bam.deleteDNSDeploymentOption(confid, "forwarding", server_id)
                            do_success = True
                        except ValueError as e:
                                logMsg = "Failure: %s Failed to delete DNS forwarding option" % ( str(e) )
                                print_log.go(log_error, logMsg)
                            ...
                            logMsg =  ('Success: delete DNS forwarding option:"%s" '
                                '[Disable forwarding for Child zone]:"%s" server:"%s" in config:"%s"') % (
                                ip_join, disable_forward_child_zones, server, configuration)
                            print_log.go(log_info, logMsg)
                            BAM.bam_logout(bam)
                            return(0)
                    break
            ...
            for ent in result_list:
                ...
                if serverGroup.upper() ==  serverGroup_name.upper(): # --- found target server to add
                    ...
                    try:
                        ent = bam.getDNSDeploymentOption(confid, "forwarding", serverGroup_id)
                        if debug_api:
                            print('[debug: ------- search result of getDNSDeploymentOption() -------]')
                            print(ent)
                    except ValueError as e:
                    ...
                    if ent['id']== 0:
                        logMsg = ('Failure: DNS forwarding option of serverGroup:"%s" in config:"%s" '
                        ...
                        try:
                            bam.deleteDNSDeploymentOption(confid, "forwarding", serverGroup_id)
                            ...
                            logMsg = ('Success: delete DNS forwarding option:"%s" '
                            '[Disable forwarding for Child zone]:"%s" in serverGroup:"%s" in config:"%s"') % (
                                ip_join, disable_forward_child_zones, serverGroup, configuration)
                            print_log.go(log_info, logMsg)
            ...
            for ent in result_list:
                ...
                if server.upper() ==  server_name.upper(): # --- found target server to add
                    try:
                        ent = bam.getDNSDeploymentOption(server_id, "forwarding", server_id)
                        if debug_api:
                            print('[debug: ------- search result of getDNSDeploymentOption() -------]')
                            print(ent)
                    ...
                    if ent['id']== 0:
                        logMsg = 'Failure: DNS forwarding option of server:"%s" does not exist' % (server)
                        print_log.go(log_error, logMsg)
                        BAM.bam_logout(bam)
                        return(1)
                        ...
                        try:
                            bam.deleteDNSDeploymentOption(server_id, "forwarding", server_id)
                            do_success = True
                        except ValueError as e:
                                logMsg = "Failure: %s Failed to delete DNS forwarding option" % ( str(e) )
                                print_log.go(log_error, logMsg)
                            ...
                            logMsg = ('Success: delete DNS forwarding option "%s" '
                                      '[Disable forwarding for Child zone]:"%s" of server:"%s"') % ( 
            ...
            for ent in result_list:
                ...
                if serverGroup.upper() ==  serverGroup_name.upper(): # --- found target server to add
                    ...
                    try:
                        ent = bam.getDNSDeploymentOption(serverGroup_id, "forwarding", serverGroup_id)
                        if debug_api:
                            print('[debug: ------- search result of getDNSDeploymentOption() -------]')
                            print(ent)
                    ...
                    if ent['id']== 0:
                        logMsg = 'Failure: DNS forwarding option of serverGroup:"%s" does not exist' % (serverGroup)
                        print_log.go(log_error, logMsg)
                        BAM.bam_logout(bam)
                        return(1)
                        ...
                        try:
                            bam.deleteDNSDeploymentOption(serverGroup_id, "forwarding", serverGroup_id)
                            do_success = True
                        except ValueError as e:
                            logMsg = "Failure: %s Failed to add DNS forwarding option" % ( str(e) )
                            print_log.go(log_error, logMsg)
                            ...
                            logMsg = ('Success: delete DNS forwarding option:"%s" '
                                  '[Disable forwarding for Child zone]:"%s" of serverGroup:"%s"') % (
                                  ip_join, disable_forward_child_zones, serverGroup)
                            print_log.go(log_info, logMsg)
