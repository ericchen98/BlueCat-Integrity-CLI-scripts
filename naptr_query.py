...
# v5.0 20210915-1500
# v3.0 2021/7/17 use pw in BAM
#

'''
naptr-query -c config1 -v view1 -z test.corp -r naptr1
naptr-query -c config1 -v view1 -z test.corp -k
...
def naptr_query(*args):    
    ...
    #----------------- (for query script to build multiple result
    logMsg2 = ''
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:z:r:i:kh", ["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:z:r:i:kh", ["debug=","version"])
        ...
        if configuration:
            ...
            if view:
                ...
                if zone:
                    ...
                    # if "absoluteName" = zone name (e.g. "test.corp" ), this is "same as zone" record
                    if sameAsZone:
                        absolutename = zone
                        rrName=""
                    else:
                    ...
                    # if -k is used, rrName will be zone name. The search result will be
                    #    "same as zone" record only.
                    
                    ent_list = bam.getEntitiesByName(zoneid, rrName, "NAPTRRecord", 0, max_obj_count)
                    
                    if len(ent_list) == 0: # no entity found
                        logMsg = "Failure: No%srecord found!" % sameAsZone_Message
                        print_log.go(log_error, logMsg)
                        
                    else: # found entity (but result could be any General record)
                    
                        logMsg2= "Success: Found%srecord %s " % (sameAsZone_Message, absolutename )
                        RecordPrintCount == 0
                        ...
                        for ent in ent_list:
                            ...
                            # build log (adding results to one line)
                            if RecordPrintCount == 1:
                                # print only when 1st record
                                print ("Info Success: Found%srecord " % (sameAsZone_Message) )
                                
                                # bluld logMsg for 1st record (without "|")
                                logMsg = " g:%s o:%s p:%s e:'%s' t:%s s:'%s'" % (properties['flags'] , \
                                    properties['order'], properties['preference'], properties['regexp'], \
                                    properties['replacement'], properties['service']) 
                            else:
                                # build logMsg with leading "|"
                                logMsg = " | g:%s o:%s p:%s e:'%s' t:%s s:'%s'" % (properties['flags'] , \
                                    properties['order'], properties['preference'], properties['regexp'], \
                                    properties['replacement'], properties['service']) 


                            # logMsg2 is for log only (append all found result to logMsg2. 
                            #       it will be sent to log)
                            logMsg2 = logMsg2 + logMsg

                            # logMsg output sample (when there are three same "naptr1" record, rdata
                            #    is seperated by "|")
                            #
                            #    2021-09-01 18:35:58,eric,vDNS,,cli,View,"[INFO]:Success: Found record
                            #    naptr1.test.corp  g:S o:300 p:10 e:'!^.*$!sip:user@sip.rfc1035.com!'
                            #    t:test1.MNC001.MCC700.gprs. s:'x-3gpp-pgw:x-gp:x-gn' | g:S o:200 p:10
                            #    e:'!^.*$!sip:user@sip.rfc1035.com!' t:test1.MNC001.MCC700.gprs.
                            #    s:'x-3gpp-pgw:x-gp:x-gn' | g:S o:100 p:10 e:'!^.*$!sip:user@sip.rfc1035.com!' 
                            #    t:test1.MNC001.MCC700.gprs. s:'x-3gpp-pgw:x-gp:x-gn'",
                            #    " Input: naptr-query-t1 -c config1 -v view1 -z test.corp -r naptr1"
                        ...
                        #end for ent in result_list:
                        
                        # to log only ( connect all IP together ) by print_log.log_only()
                        if not RecordPrintCount == 0:
                            print_log.log_only(log_info, logMsg2)
                        else:
                            # if no record match in for loop, screen has no output. print_log.go() 
                            #   - print to scr & log
