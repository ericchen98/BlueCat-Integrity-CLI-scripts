...
#
# program name: cname-query.py
# by: Eric Chen (hchen@bluecatnetworks.com)
# python ver: python 3
# BAM ver: 26.1
#
# v1.0-20260918 (based on a_query.py's style)

'''
cname-query -c config1 -v view1 -z test.corp -r www
    ...
    # query a cname (alias) record from a zone
'''
import getopt
import sys
import json
...
def cname_query(*args):
    ...
    #----------------------------------------------------------------
    #get program name. If prog is called by cli.py, then __name__ will be "cname_query"
    prog_name = __name__

    #convert "_" to "-" in prog_name
    prog_name = prog_name.replace('_','-')

    if __name__ == '__main__': #this program is run by python directly (e.g. python3 cname_query.py)
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:z:r:h", ["debug=","version"])
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:z:r:h", ["debug=","version"])
                ...
                if zone:
                    ...
                    if len(result_list) == 0: # no entity found
                        ...
                        for ent in result_list:
                            ...
                            # print (properties)
                            #     output : {'type': 'CNAME', 'absoluteName': 'www.test.corp', 'linkedRecordName': 'test.corp'}

                            if properties['type']=="CNAME":

                                RecordPrintCount +=  1

                                print ("Info Success: Found%srecord: %s -> %s " %
                                        (sameAsZone_Message, properties['absoluteName'], properties['linkedRecordName']) )
                                query_ok_flag = True

                                if RecordPrintCount == 1:
                                    logMsg = "linkedRecordName:%s " % ( properties['linkedRecordName'] )
                                else:
                                    logMsg = "| linkedRecordName:%s " % ( properties['linkedRecordName'] )

                            # logMsg2 is for log only (append all result to logMsg2)
                            logMsg2 = logMsg2 + logMsg
                        #end for ent in result_list:

                        # to log only ( connect all linked records together )
