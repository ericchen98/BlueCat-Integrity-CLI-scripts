...
'''
srv-delete -c config1 -v view1 -z test1.corp -r srv1 -i host1.test.corp -o 10 -p 5060 -w 100
...
def srv_delete(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "hdc:v:z:r:i:o:p:w:k",["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "hdc:v:z:r:i:o:p:w:k",["debug=","version"])
                    ...
                    if len(result_list) == 0: # no entity found
                        ...
                        for ent in result_list:                 # ent is also json too
                            ...
                            if debug:
                                ...
                                print (absolutename)
                                '''
                                if properties['absoluteName'] == absolutename:
                                    print ("      absoluteName match")
                                else:
                                    print ("      absoluteName NOT match")
                                '''
                                print ('--------------------------------------linkedRecordName(exist vs input)')
                                print (properties['linkedRecordName'])
                                print (linkedRecordName)
                                '''
                                if properties['linkedRecordName'] == linkedRecordName:
                                    print ("      linkedRecordName match")
                                else:
                                    print ("      linkedRecordName NOT match")
                                '''
                            ...
                            if properties['linkedRecordName'] == linkedRecordName and \
                                ...
                                try:
                                    bam.delete (ent['id'])
                                    prnMsg = "Success: delete%srecord %s" % ( 
                                         sameAsZone_Message, absolutename)
                                    print (prnMsg)
                                    record_changed = True
                                    
                                except ValueError as e:
                                    logMsg = "Failure: %s Failed to delete%srecord: %s" % ( str(e), 
                                                                        sameAsZone_Message, absolutename)
                                    print_log.go(log_error, logMsg)
                                    record_changed = False
                                    error_Logged = True
                        ...
                        if record_changed:
                            print ()
                            logMsg =  "Success: delete%srecord %s " % ( sameAsZone_Message, 
                                                                    absolutename)
                            ...
                            if not error_Logged:
                                ...
                                logMsg = "Failure: failed to delete%srecord %s" % ( sameAsZone_Message, 
                                                                            absolutename)
                                print_log.go(log_error, logMsg)
                                BAM.bam_logout(bam)
                                return(1)
