...
'''
dhcp4range_add -c config1 -i 10.1.1.0/24 -s 10.1.1.11 -e 10.1.1.20 -n range1
     ...
     -n name                    # (optional) dhcpv4 range name
'''
...
def dhcp4range_add(*args): 
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:i:s:e:n:",["ni=","debug=","dry", "version"])
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:i:s:e:n:",["ni=","debug=","dry", "version"])
            ...
            if confid == 0:
               ...
               # ent found, but need to tell whether it is in same config!!!
                ...
                if len(result_list) == 0: # no entity found
                    ...
                    try:
                        rtn = bam.addDHCP4Range(id, start_ip, end_ip, prop_to_add)
                        
                        #logMsg = "Success: added dhcp v4 Range. Network: %s start ip:%s End ip: %s" % (
                        #        ipNetwork,start_ip, end_ip)

                        logMsg = "Success: added dhcp v4 Range. Network: %s Range: %s-%s" % (
                                ipNetwork,start_ip, end_ip)
