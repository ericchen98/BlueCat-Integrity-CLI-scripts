...
# v5.5-20230827 update help and version info
# v5.4-20221202
#       1) modified: logPath = "./"
...
'''
a-delete -c config1 -v view1 -z test.corp -r pc1 -i 10.10.10.3
a-delete -c config1 -v view1 -z test.corp -i 10.10.10.3 -k
    ...
    # delete an A record from a zone
'''
...
def a_delete(*args): 
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(input_args,   "hdc:v:z:r:i:k", ["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "hdc:v:z:r:i:k", ["debug=","version"])
                ...
                if zone:
                    ...
                    if len(result_list) == 0: # no entity found
                        ...
                        for ent_json in result_list:
                            ...
                            # print (" ent['id'] = {} ".format(ent['id']))
                            # return dict properity : 
                            #       e.g. {'absoluteName': 'a1.test.corp', 'type': 'A', 'rdata': '1.1.1.1'}
                            ...
                            if properties['type']=="A":
                                ...
                                if properties['rdata'] == rdata:
                                    ...
                                    absoluteName_str1 = properties['absoluteName']
                                    rdata_str1 = properties['rdata']
                                    print ( f"(*) Record: {absoluteName_str1}, Rdata: {rdata_str1} -> deleting...")
                                                    
                                    try:
                                        bam.delete( ent_json['id'] )
                        ...
                        # end of for ent_json in result_list:
                        ...
                        if not record_changed:
                            ...
                            # record deleted!. exit with error_code 0 
                            # put success message here is better
                            logMsg = "Success:%srecord deleted %s" % (sameAsZone_Message, absolutename)
                            print_log.go(log_info, logMsg)
                            BAM.bam_logout(bam)
                            return (0)

                    # end of if len(result_list) == 0: # no entity found
