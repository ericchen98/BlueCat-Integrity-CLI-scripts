...
# BAM ver: 26.1
#
# v1.4-20240706 add -c config to help
# v1.3-20230828 fixed log does not show in cli.py issue 
# v1.2 update help
# v1.1 update return value
...
'''
rpz-add.py -c config -r ReponsePolicy-1  -i a1.test.corp 

    -c config                   # config name
    -r ReponsePolicy_name       # RPZ policy name
    -i a1.test.corp             # RPZ policy item to be added

    # Add a RPZ policy item to a BAM RPZ ReponsePolicy
'''
import getopt
import sys
import json
...
def rpz_add(*args): 
    ...
    passed_args = " [arguments]: rpz_add" + passed_args
    #print(passed_args)
    ...
    try:    #for general api exception
        if configuration:
            ...
            rtn = bam.addResponsePolicyItem(rpz_ent_id, rpz_item, options)
            
            #return 'true' if add success
            #return 'false' if item exists already

            if rtn.upper() == "TRUE":
                # add success
                logMsg = "Success: add Response Policy item %s"  % ( rpz_item )
                print_log.go(log_info, logMsg )
            else:
                # return false: item exists already
                logMsg = "False: item exists already: %s"  % ( rpz_item )
                print_log.go(log_error, logMsg )
            BAM.bam_logout(bam)
            return(1) 
