...
# v5.5-2023-0827 - replace sys.exit() to return() (to work with cli.py)
#                - update help
# v5.4-20221220
#       1) modified: logPath = "./"
...
#
# note: A, AAAA, NS are Generic records
#       srv, naptr, host are NOT Generic records

'''
generic-query -c config1 -v view1 -z test.corp -r pc1        #list all types of records with name pc1
generic-query -c config1 -v view1 -z test.corp -r pc1 -t NS  #list only type NS with name pc1
generic-query -c config1 -v view1 -z test.corp -k
    -c config_name
    -v view_name
    -z zone_name
    -r record_name
...
def generic_query(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:z:r:t:i:kh", ["debug=","version"])
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:z:r:t:i:kh", ["debug=","version"])
            ...
            if view:
                ...
                if zone:
                    ...
                    if len(result_list) == 0: # no entity found
                        ...
                        for ent in result_list:
                            ...
                            if rrtype == None:
                                print ( "Record: " + properties['absoluteName'] + ","  \
                                        ...
                                        + " rdata: " + properties['rdata'])
                                RecordPrintCount = RecordPrintCount+1
                            else:
                                if rrtype == properties['type']:
                                    print ( "Record: " + properties['absoluteName'] + ","  \
                                            + " type: " + properties['type'] + "," \
                                            + " rdata: " + properties['rdata'])
                                    RecordPrintCount = RecordPrintCount+1
                        ...
                        logMsg =  "Success: query generic record" 
                        print_log.go(log_info, logMsg)
                        
                    #endif len(result_list) == 0: # no entity found
                    BAM.bam_logout(bam)                    
                    return (0)
