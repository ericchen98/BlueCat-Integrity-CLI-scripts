...
'''
naptr-delete -c config1 -v view1 -z test.corp -r naptr1 -g S -o 100 -p 0 -e !^.*$!sip:eric@sip.rfc1035.com! -t test1.MNC001.MCC700.gprs. -s x-3gpp-pgw:x-gp:x-gn
    ...
    Note: use single quote (') to quote special characters in parameters.
          E.g. RegEx needs to use single quote (') to quite it.
...
def naptr_delete(*args):    
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:v:z:r:g:o:p:e:t:s:kh", ["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:v:z:r:g:o:p:e:t:s:kh", ["debug=","version"])
                        ...
                        for ent in result_list:                 # ent is also json too
                            ...
                            if properties['absoluteName'] == absolutename and \
                               ...
                               properties['service'] == service:
                                ...
                                # all fields matched. delete record
                                ...
                                print ( "(*) Record: " + properties['absoluteName'] + ","  \
                                    ...
                                    + " service: " + properties['service'] + " -> deleting..." )
                                
                                # update all fields with new input
                                
                                if debug:
                                    print ( '------ Entity to be delete by bam.delete(ent) -------')
                                    print (ent)
                                    print ('\n')
                                    # output
                                ...
                                try:
                                    bam.delete (ent['id'])
                                    prnMsg = "Success: delete%srecord %s" % (sameAsZone_Message, absolutename)
                                    print (prnMsg)
                                    record_changed = True
