...
# BAM ver: 26.1
#
# v5.5-20240706 fixed parameter does not work with cli.py issue
# v5.4-20230828 fixed log does not show in cli.py issue 
# v5.3 use uploadResponsePolicyFile() instead of uploadResponsePolicyItem()
# v5.2 update req_timeout from 3sec to 120sec. 
#       Note: VM BAM test result, 30sec (upload 500K(9MB size) item OK, upload 1M(19MB) item fail)
#       Note: VM BAM test result, 60sec (upload 1M(19MB size) item OK
#       Note: VM BAM test result, 60sec (upload 1.4M(25MB size) item OK
...
'''
rpz-upload.py -c config1 -r ReponsePolicy-1  -f rpz-list.txt 

    -c config_name
    -r ReponsePolicy_name
    -f rpz-list.txt 

    # Upload RPZ items in a text file to BAM RPZ policy 
    # Note: this program will replace all existing rpz policy item with new list
'''
import getopt
import sys
import json
...
def rpz_upload(*args): 
    ...
    try:
        ...
        if len(input_args)!=0:    #this is called by other python script
            opts, args = getopt.getopt(  input_args, "dc:z:r:f:h",["debug=","version"])            
        else: # call this script from CLI. Parsing "sys.argv[1:]" ([1:] - not include script name)
            opts, args = getopt.getopt(sys.argv[1:], "dc:z:r:f:h",["debug=","version"])
    ...
    if (not fileName) or (fileName[0:1] == "-"):
        logMsg = "Failure: -f fine_name option is required"
        print_log.go(log_error, logMsg)
        return(1)
    else:
        if not os.path.isfile(fileName):
            logMsg = "Failure: file %s cannot be found" % (fileName)
            print_log.go(log_error, logMsg)
            return(1)
    # ----------------------------------------------------------------
    # login to BAM
    ...
    try:    #for general api exception
        if configuration:
            ...
            try:
                ...
                if rpz_ent_id == 0:
                    logMsg = "Failure: fail to get rpz %s" % rpz
                    print_log.go(log_error, logMsg)
                    BAM.bam_logout(bam)
                    return(1)
                    
            except ValueError as e:
                logMsg = "Failure: %s fail to get RPZ" % str(e)
                print_log.go(log_error, logMsg)
                BAM.bam_logout(bam)
                return(1)

            #rtn = bam.uploadResponsePolicyItems( fileName, rpz_ent_id)
            rtn = bam.uploadResponsePolicyFile( fileName, rpz_ent_id)

            logMsg = "Success: uploaded rpz file %s"  % ( fileName)
